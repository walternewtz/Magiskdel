...
// credit: http://github.com/yujincheng08
...
static std::vector<MountInfo> ParseMountInfo(const char *pid, bool include_root = true,
    const char **include_list = nullptr, const char **exclude_list = nullptr) {
    char buf[PATH_MAX] = {};
    ...
    while ((nread = getline(&line, &len, mount_info.get())) != -1) {
        ...
        if (include_root && target == "/")
            goto skip_check_list;
        if (include_list != nullptr) {
            add = false;
            for (int i=0; include_list[i]!=nullptr;i++){
                if (target == include_list[i] || target.starts_with(std::string(include_list[i]) + "/")){
                    add = true;
                    break;
                }
            }
        ...
        if (exclude_list != nullptr) {
            for (int i=0; exclude_list[i]!=nullptr;i++){
                if (target == exclude_list[i] || target.starts_with(std::string(exclude_list[i]) + "/")){
                    add = false;
                    break;
                }
            }
